...
export default function ConnectPanel() {
  ...
  function onSubmit(e: FormEvent) {
    ...
    if (!password.trim()) {
      setMsg("Zadejte heslo (uloží se jen lokálně až po zapnutí live sync — teď se nikam neposílá).");
      return;
    }
  ...
  return (
    <form className="connect-form" onSubmit={onSubmit} noValidate>
      ...
      <div className="notice" role="note" style={{ marginBottom: "1rem" }}>
        ...
        Teď se údaje nikam neodesílají — jen kontrola formuláře. MVP zůstává{" "}
        <strong>upload CSV/XLSX</strong>, <strong>bez auto-prodloužení</strong>.
      </div>
      ...
      <div className="cta-row">
        ...
        <button type="button" className="btn" disabled title="Live sync přijde v další iteraci">
          Připojit (brzy)
        </button>
      </div>

      {msg && (
        <div className={ok ? "ok-box" : "error"} role="status" style={{ marginTop: "1rem" }}>
          {msg}
        </div>
      )}
    </form>
